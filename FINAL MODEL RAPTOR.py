...
class TrafficSimulation:
    ...
    def update_system_metrics(self):
        ...
        if self.intersections:
            ...
            self.performance_history.append(self.avg_performance)
            ...
            self.efficiency_history.append(self.system_efficiency)
            ...
            self.learning_history.append(self.learning_progress)
    ...
    def draw_performance_graph(self, x, y, width, height):
        """Draw performance graph with actual data"""
        # Graph background
        graph_rect = pygame.Rect(x, y, width, height)
        pygame.draw.rect(self.screen, (*GRADIENT_DARK, 150), graph_rect, border_radius=8)
        pygame.draw.rect(self.screen, ACCENT_BLUE, graph_rect, 2, border_radius=8)
        
        # Graph title
        title_surf = self.font.render("System Performance", True, WHITE)
        ...
        if len(data_points) > 1:
            # Scale data to graph
            max_val = max(data_points) if data_points else 10
            ...
            # Draw grid lines
            for i in range(0, 6):
                grid_y = graph_y_start + (graph_height * i) // 5
                pygame.draw.line(self.screen, (*LIGHT_GRAY, 50), 
                               (x + 10, grid_y), (x + width - 10, grid_y), 1)
            
            # Draw performance line
            for i in range(1, len(data_points)):
                x1 = x + 10 + (i - 1) * graph_width // max(len(data_points) - 1, 1)
                x2 = x + 10 + i * graph_width // max(len(data_points) - 1, 1)
                
                y1 = graph_y_start + graph_height - int((data_points[i-1] - min_val) / (max_val - min_val) * graph_height)
                y2 = graph_y_start + graph_height - int((data_points[i] - min_val) / (max_val - min_val) * graph_height)
                
                pygame.draw.line(self.screen, ACCENT_GREEN, (x1, y1), (x2, y2), 2)
                if i == len(data_points) - 1:  # Last point
                    pygame.draw.circle(self.screen, NEON_GREEN, (x2, y2), 3)
            
            # Display current value
            if data_points:
                current_val = data_points[-1]
                val_text = self.font.render(f"Current: {current_val:.2f}", True, WHITE)
                self.screen.blit(val_text, (x + width - 120, y + 25))
    ...
    def draw_live_graphs(self):
        ...
        self.draw_mini_graph(graph_x, graph_y + mini_height + 10, mini_width, mini_height,
                            list(self1.performance_history), "Performance", CYAN)
    ...
    def spawn_emergency_vehicle(self):
        ...
        x, y, direction = random.choice(spawn_points)
        target = random.choice(self.intersections)
        car = Car(x, y, direction, 3.5, target, 'emergency')
        self.cars.append(car)
        self.total_cars_spawned += 1
        self.emergency_vehicles += 1
